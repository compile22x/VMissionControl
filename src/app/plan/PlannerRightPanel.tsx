...
export function PlannerRightPanel({
  ...
  const activePlanName = p.activePlanId ? p.missionName || "Untitled Plan" : null;

  return (
    <div className="w-[320px] shrink-0 flex flex-col border-l border-border-default bg-bg-secondary">
      <div className="flex items-center justify-between px-3 py-2 border-b border-border-default">
        <div className="flex items-center gap-1.5 min-w-0">
          {p.isDirty && <span className="w-1.5 h-1.5 rounded-full bg-status-warning shrink-0" title="Unsaved changes" />}
          <h2 className="text-sm font-display font-semibold text-text-primary truncate">{activePlanName || "Mission Planner"}</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <CollapsibleSection title="Waypoints" defaultOpen={true} count={p.waypoints.length}
        ...
        {showGeofence && (
          <CollapsibleSection title="Geofence" trailing={<span className="text-[10px] font-mono text-text-tertiary">{p.geofenceEnabled ? "on" : "off"}</span>}>
        ...
        {showRally && (
          <CollapsibleSection title="Rally Points" count={p.rallyPoints.length}>
        ...
        <CollapsibleSection title="Transform"><TransformPanel /></CollapsibleSection>
